...
# MÉTRICAS GERAIS (fora das abas, sempre visível)
...
# ABAS
tab1, tab2 = st.tabs(["🏆 Rankings", "📊 Gráficos & Histórico"])
...
with tab1:
    ...
    ranking_individual = df.groupby(["Nome", "Grupo"])["Pontos"].sum().reset_index()
    ranking_individual = ranking_individual.sort_values("Pontos", ascending=False)
    ranking_individual.insert(0, "Posição", range(1, len(ranking_individual) + 1))
    ...
    ranking_grupos = df.groupby("Grupo")["Pontos"].sum().reset_index()
    ranking_grupos = ranking_grupos.sort_values("Pontos", ascending=False)
    ranking_grupos.insert(0, "Posição", range(1, len(ranking_grupos) + 1))
    st.dataframe(ranking_grupos, use_container_width=True)

    st.divider()

    # Ranking por Tipo de Pontos
    st.subheader("🎯 Ranking por Tipo de Pontos")
    if "Descricao" in df.columns:
        tipos_pontos = sorted(df["Descricao"].dropna().unique())
        tipo_selecionado = st.selectbox("Escolha o tipo de pontuação", tipos_pontos)
        df_tipo = df[df["Descricao"] == tipo_selecionado]
        ranking_tipo = df_tipo.groupby("Nome")["Pontos"].sum().reset_index()
        ranking_tipo = ranking_tipo.sort_values("Pontos", ascending=False)
        ranking_tipo.insert(0, "Posição", range(1, len(ranking_tipo) + 1))

        col1, col2 = st.columns([3, 1])
        with col1:
            st.dataframe(ranking_tipo, use_container_width=True)
        with col2:
            st.subheader("🏆 Top 3")
...
with tab2:
    ...
    grupo_total = df.groupby("Grupo")["Pontos"].sum().reset_index()
    fig = px.bar(
        grupo_total,
        x="Grupo",
        y="Pontos",
    ...
    trimestre_grupo = df.groupby(["TrimestreNome", "Grupo"])["Pontos"].sum().reset_index()
    fig2 = px.line(
        trimestre_grupo,
        x="TrimestreNome",
        y="Pontos",
    ...
    st.subheader("📜 Histórico por Trimestre")
    historico = df.groupby(["TrimestreNome", "Grupo"])["Pontos"].sum().reset_index()
    ...
    for trimestre in sorted(trimestres):
        ...
        st.success(f"🥇 Campeão: {campeao['Grupo']} — {campeao['Pontos']} pontos")
        for i, row in bloco.iterrows():
            st.write(f"{row['Grupo']} — {row['Pontos']} pontos")
        st.divider()
